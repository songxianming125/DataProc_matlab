function [info,picture,t]=downloadcine(shotnum,t1,t2)
...
% example [info,images,times]=cine_info()

shotnum_name=[num2str(shotnum),'.cine'];
cd('\\diskstation\exl50-camera');
temp=dir('*');
% temp=dir('20200409/*.cine');
% index = find(strcmp({temp.name}, shotnum_name)==1);
% cinefilename=[temp(index,:).folder,'\',num2str(shotnum),'.cine'];

cinefilename=['20200409\',num2str(shotnum),'.cine'];
