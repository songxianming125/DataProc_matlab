function  [y,x,Unit,varargout] = eastFormula(CurrentShot,CurrentChannel,varargin)
global handles
Unit='';
z=[];
...
switch lower(CurrentChannel)
    case '@dh'
        chnls={'pcpf1','pcpf2','pcpf3','pcpf4'};
        [z,x]=eastdbN(CurrentShot,chnls,varargin{:});
        Unit='cm';
        y=((z(:,1)+z(:,3))-(z(:,2)+z(:,4)))/2;
    otherwise
        switch lower(CurrentChannel(1))
            ...
            case '_'
                [y,x,Unit]=eastdb(CurrentShot,CurrentChannel(2:length(CurrentChannel)));
